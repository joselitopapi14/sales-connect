...
export function ContainerTextFlip({
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      // Update width after changing word
      updateWidthForWord();
    }, interval);

    return () => clearInterval(intervalId);
  }, [words.length, interval, updateWidthForWord]);
  ...
  return (
    <motion.p
      ...
      className={cn(
        "relative inline-block rounded-lg pt-2 pb-3 text-center text-4xl font-bold text-slate-700 md:text-7xl dark:text-white",
      ...
      >
        <motion.div className="inline-block">
          {words[currentWordIndex].split("").map((letter, letterIndex) => (
            <motion.span
              key={`${words[currentWordIndex]}-${letterIndex}`}
              initial={{
                opacity: 0,
                filter: "blur(10px)",
              }}
